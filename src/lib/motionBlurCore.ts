// Motion-blur core: GPU shader, WebGL setup, and CPU motion estimation.
//
// Extracted from MotionBlurCanvas.tsx (Phase 2) so the same directional
// motion-blur primitives can be shared by both the live preview (RAF loop in
// the React component) and the offscreen export renderer. Keeping these out of
// the component file also resolves the React Fast Refresh warning that fires
// when a module exports both a component and non-component values.
//
// Nothing here touches React — it is pure WebGL + typed-array math so it can be
// driven headlessly during MP4 export.
...
function devError(...args: unknown[]): void {
  if (!isDev) return;
  // no-console is not enabled in this project's eslint config; this dev-only
  // path is stripped from production by the isDev guard above.
  console.error(...args);
}
...
/**
 * Build the WebGL program + fullscreen-quad buffer + ping-pong textures and
 * resolve all uniform locations. Works against any canvas that can hand back a
 * `webgl` context — an on-screen `<canvas>` for the live preview or an
 * `OffscreenCanvas` / detached canvas for headless export rendering.
 */
...
/**
 * Gamma curve for the motion-blur strength multiplier.
 *
 * The intensity stored on the clip is linear 0..100 from the effects panel.
 * Players experience that as "nothing happens 0-30, then it pops between
 * 70-90, then breaks". A gamma curve <1 pushes more perceptible change into
 * the middle of the range so dragging 40 → 60 actually feels like a doubling.
 * The peak is capped below the regime where the line integral exceeds the
 * actual scene shift.
 *
 * Shared by the live preview and the export renderer so a clip's authored
 * intensity maps to the SAME shader strength in both — without this the export
 * would blur by a different amount than the preview showed.
 */
export const STRENGTH_GAMMA = 0.6; // <1 → boost the low end
export const STRENGTH_PEAK = 1.25; // tighter cap than the previous 1.5

/** Map a 0..1 normalised intensity to the shader strength multiplier. */
export function shapeStrength(intensity01: number): number {
  return Math.pow(Math.max(0, Math.min(1, intensity01)), STRENGTH_GAMMA) * STRENGTH_PEAK;
}

/**
 * Derive the export blur strength from a clip's stored 0..100 intensity and its
 * playback speed. Mirrors the preview's `shapeStrength(i/100) * speedFactor`
 * (the preview additionally applies a spot-check `previewBlurBoost`, which is
 * intentionally NOT part of the authored/export strength).
 *
 * Faster playback compresses more scene motion into each output frame, so the
 * blur scales with speed — exactly as the preview does.
 */
export function exportStrengthFromIntensity(intensity0to100: number, speed: number): number {
  const intensity = Math.max(0, Math.min(100, intensity0to100));
  // Same clamp the preview applies: speedFactor = clamp(clipSpeed, 0.5, 2).
  const speedFactor = Math.max(0.5, Math.min(2, speed));
  return shapeStrength(intensity / 100) * speedFactor;
}

// Diff thresholds in 0..1 luma space, shared by preview and export so both
// paths classify "static UI vs moving scene" identically. Anything below
// MASK_LOW reads as static and stays sharp; above MASK_HIGH gets the full
// blur. 0.04 / 0.12 keeps UI (≤ a few %) sharp while modest gameplay
// contrast changes hit the blur path.
export const MASK_LOW = 0.04;
export const MASK_HIGH = 0.12;

/**
 * Per-frame uniform inputs for a single motion-blur draw. Everything the
 * shader needs that varies frame-to-frame; the static program/buffer/textures
 * live in {@link GlSetup}.
 */
export interface RenderFrameParams {
  /** Motion vector in UV space, already scaled by the strength slider. */
  motionUVX: number;
  motionUVY: number;
  /** Length of the motion vector — gates the shader's early-out. */
  magnitudeUV: number;
  /** Adaptive sample count, clamped to [SAMPLES_MIN, SAMPLES_MAX]. */
  sampleCount: number;
  /** HUD positional-protect weight, 0..1. */
  hudMask: number;
  /** HUD preset index (see HUD_PRESET_INDEX). */
  hudPresetIndex: number;
  /** Viewport aspect (w/h) for HUD layout. */
  aspect: number;
  /** Whether the prev-frame texture holds real data (false on first frame). */
  hasPrevFrame: boolean;
}

/**
 * Issue one motion-blur draw using already-uploaded textures.
 *
 * Caller contract: TEXTURE0 must already have the current frame uploaded into
 * `setup.curTexture`, and (when `hasPrevFrame`) `setup.prevTexture` holds the
 * previous frame. This function binds the textures, sets every uniform, and
 * draws the fullscreen quad into whatever framebuffer is currently bound (the
 * canvas backing store by default).
 *
 * Shared verbatim by the live preview RAF loop and the offscreen export
 * renderer so the GPU result is byte-identical between preview and export.
 */
export function renderFrame(
  ...
  // On the very first frame there's no prev yet — fall back to the current
  // texture, which makes the diff zero everywhere → no blur applied (sharp).
  gl.activeTexture(gl.TEXTURE1);
  gl.bindTexture(gl.TEXTURE_2D, params.hasPrevFrame ? prevTexture : curTexture);
  gl.uniform1i(uPrevFrame, 1);

  gl.uniform2f(uMotion, params.motionUVX, params.motionUVY);
  gl.uniform1f(uMagnitude, params.magnitudeUV);
  gl.uniform1f(uMaskLow, MASK_LOW);
  gl.uniform1f(uMaskHigh, MASK_HIGH);
  gl.uniform1f(uHudMask, params.hudMask);
  gl.uniform1i(uHudPreset, params.hudPresetIndex);
  gl.uniform1f(uAspect, params.aspect);
  gl.uniform1i(uSamples, params.sampleCount);
...
/**
 * Convert a smoothed motion vector (in SAD sample-cell units) into the UV-space
 * uniforms + adaptive sample count the shader expects. Shared so preview and
 * export derive identical magnitudes from the same measured motion.
 */
export function motionToUniforms(
  smoothDx: number,
  smoothDy: number,
  strength: number,
): { motionUVX: number; motionUVY: number; magnitudeUV: number; sampleCount: number } {
  // SAMPLE_W cells span the whole frame, so 1 cell = 1/SAMPLE_W of UV.
  const motionUVX = (smoothDx / SAMPLE_W) * strength;
  const motionUVY = (smoothDy / SAMPLE_H) * strength;
  const magnitudeUV = Math.hypot(motionUVX, motionUVY);

  // Adaptive sample count — more samples for bigger blurs (avoids banding)
  // and fewer for tiny ones (saves fragment work). Scale by the UV magnitude
  // relative to the 8% clamp.
  ...
  return { motionUVX, motionUVY, magnitudeUV, sampleCount };
}
